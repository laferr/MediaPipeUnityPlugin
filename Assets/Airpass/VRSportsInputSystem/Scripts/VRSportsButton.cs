...
namespace Airpass.VRSports
{
    ...
    public class VRSportsButton : MonoBehaviour, IVRSportsInteractable
    {
        ...
        public float localHoldDelay = 2.0f;
        ...
        void Charging()
        {
            if (!holdingOnce)
            {
                ...
                OnHolding.Invoke();
                if(Time.timeScale == 0) {
                    if(localHoldDelay == VRSportsInputSystem.Instance.holdDelayOfVRSportsButton)
                        img_holding.fillAmount += ((holdFlip ? -1 : 1) * (Time.unscaledDeltaTime / VRSportsInputSystem.Instance.holdDelayOfVRSportsButton));
                    else
                        img_holding.fillAmount += ((holdFlip ? -1 : 1) * (Time.unscaledDeltaTime / localHoldDelay));
                }
                else
                {
                    if (localHoldDelay == VRSportsInputSystem.Instance.holdDelayOfVRSportsButton)
                        ...
                        img_holding.fillAmount += ((holdFlip ? -1 : 1) * (Time.deltaTime / localHoldDelay));
                }
