...
    public partial class VRSportsInputSystem
    {
        ...
        public static Dictionary<int, InteractData> interactDatas = new();
...
            switch (data.status)
            ...
            switch (data.type)
            {
                ...
                case InteractType.holding: @interface.InteractHolding(data); break;
        ...
        public bool RaycastPhysicsHit2D(Vector2 _position, Camera _cam, float _maxDistance, out RaycastHit2D _hit)
        {
            _hit = Physics2D.Raycast(_cam.ScreenToWorldPoint(_position), Vector2.zero);
...
            Camera cam = physicsRaycastCamera;
            ...
            {
                if (RaycastPhysicsHit2D(data.position, cam, Mathf.Infinity, out RaycastHit2D hit2D))
                {
                    //Debug.Log($"hitted : {hit2D.transform.gameObject.name}");
                    if (hit2D.transform.TryGetComponent(out IVRSportsInteractable _))
                    {
                        InvokeIVRSportsInteractable(data, hit2D.transform.gameObject);
                    }
                }
            }
        }

        public void FinishVRSportsEvent()
        {
...
            foreach (KeyValuePair<IVRSportsInteractable, InteractableDummy> t in interactedList)
        ...
        private void ClearVRSportsEvent()
        {
            foreach (KeyValuePair<IVRSportsInteractable, InteractableDummy> t in interactedList)
            {
                InteractableDummy dummy = t.Value;
...
        private IEnumerator UdpReceiver()
        {
            while (true)
            {
                //Debug.Log(interactDatas.Count);
                if (udpClientReceive == null) break;
                currentReceivedLidar.Clear();

                // Remove status up touch.
                List<int> keys = interactDatas.Keys.ToList();
                foreach (int key in keys)
                {
                    if (interactDatas[key].status == "2")
                    {
                        interactDatas.Remove(key);
                    }
                }
                    ...
                    if (enable)
                    {
                        ...
                        if (byteData != null)
                        {
                            ...
                            for (int i = 0; i < jArray.Count; ++i)
                            {
                                ...
                                {
                                    //if (t.status == "1" || (t.status == "0" && temp.status == "2"))
                                    //{
        ...
        void OnGUI()
        {
            if (enable && debugging)
            {
                ...
                else
                {
                    ...
                    {
                        text += ($"{data.Value.id}[{data.Value.status}] : {data.Value.position}{Environment.NewLine}");
                    }
                }
